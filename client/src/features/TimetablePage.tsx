...
export const TimetablePage: React.FC = () => {
  ...
  const rooms = [
    { id: 'r1', name: 'Aula 101', type: 'room' as const },
    { id: 'r2', name: 'Laboratorio 1', type: 'room' as const },
    { id: 'r3', name: 'Gimnasio', type: 'room' as const },
  ];

  const subjects = [
    { id: 's1', name: 'Matemáticas', type: 'subject' as const },
    { id: 's2', name: 'Física', type: 'subject' as const },
    { id: 's3', name: 'Lengua', type: 'subject' as const },
  ];
  ...
  useEffect(() => {
    ...
    socketService.joinTimetable('demo-timetable', guestUser.id, guestUser.name);
    ...
    return () => {
      socketService.leaveTimetable('demo-timetable', guestUser.id, guestUser.name);
    };
  }, [guestUser]);
  ...
  const handleSlotDrop = (day: string, periodId: string, item: any) => {
    ...
    const newSlot = {
      ...
      day,
      periodId,
      subject: item.name || item.subject || 'Nueva Asignatura',
      teacher: item.teacher || teachers[0]?.name || 'Profesor',
      room: item.room,
      color: '#3b82f6',
      conflicts,
      warnings: [],
    ...
    socketService.emitUpdate('demo-timetable', guestUser.id, guestUser.name, {
      type: 'add',
      slot: newSlot,
    });
  };
          ...
          <div className="flex-1 px-4 sm:px-6 lg:px-8 py-8">
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
                <div>
                  ...
                  <p className="text-gray-600 dark:text-gray-400 mt-2">
                    Arrastra y suelta recursos para crear tu horario
                  </p>
                </div>
            ...
            <div className="mt-8 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">
                🤖 Sugerencias de Programación IA
              </h2>
              <div className="space-y-3">
                <SuggestionCard
                  suggestion="Clase de Matemáticas para 10º mejor el lunes a las 9:00 - Sin conflictos detectados"
                  score={95}
                  type="success"
                />
                <SuggestionCard
                  suggestion="Aviso: La profesora María tiene 4 períodos consecutivos el martes - Considere añadir descanso"
                  score={72}
                  type="warning"
                />
                <SuggestionCard
                  suggestion="Laboratorio 1 disponible para Física el miércoles por la mañana - Buen ajuste para trabajo práctico"
                  score={88}
                  type="info"
                />
              </div>
